...
function greyConverter() {
    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[j].length; j++) {
            let pixel = pixels[i][j];
            let media = pixel.reduce((sum,value) => sum + value, 0);
            let grey = Math.round(media/3);
            pixel[0]=grey;
            pixel[1]=grey;
            pixel[2]=grey;
        }
...
function scaleDown() {
    ...
    pixels = pixels.map((row) => row.filter((column, index) => index % 2 !== 0));
...
function dimBrightness(dimFactor) {
    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            ...
            pixel[0] = Math.max(0, pixel[0] / dimFactor);
            pixel[1] = Math.max(0, pixel[1] / dimFactor);
            pixel[2] = Math.max(0, pixel[2] / dimFactor);
...
function merge(alphaFirst, alphaSecond) {
    ...
    for (let i = 0; i < height; i++) {
        ...
        for (let j = 0; j < width; j++) {
            ...
            let newPixel = [
                Math.min(255, Math.max(0, (catPixel[0] * alphaFirst) + (dogPixel[0] * alphaSecond))),
                Math.min(255, Math.max(0, (catPixel[1] * alphaFirst) + (dogPixel[1] * alphaSecond))),
                Math.min(255, Math.max(0, (catPixel[2] * alphaFirst) + (dogPixel[2] * alphaSecond)))
            ];
