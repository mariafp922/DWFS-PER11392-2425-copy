...
function greyConverter() {
  ...
  for (let i=0; i<pixels.length; i++){
    for (let j=0; j<pixels[i].length; j++){
    pixels[i][j][0]=pixels[i][j][1]=pixels[i][j][2]=(pixels[i][j][0]+pixels[i][j][1]+pixels[i][j][2])/3;
    }
  }
...
function blackAndWhiteConverter() {
  ...
  for (let i=0; i<pixels.length; i++){
    for (let j=0; j<pixels[i].length; j++){
      if (((pixels[i][j][0]+pixels[i][j][1]+pixels[i][j][2])/3)<128){
...
function scaleDown() {
  ...
  let pixels = handler.getPixels();
  let pixels_minimizado=[];


  //Aqui tu codigo
  for (let i=0; i<pixels.length; i++){
    if (i%2==0){
      pixels_minimizado[i/2]=pixels[i];
    }
  }

  for (let i=0; i<pixels_minimizado.length; i++){
    for (let j = pixels_minimizado[i].length - 1; j >= 0; j--){
         if (j%2==0){
            pixels_minimizado[i].splice((j),1);
          }
      }
    }


  handler.savePixels(pixels_minimizado, outputPath, pixels_minimizado.length, pixels_minimizado[1].length);
}
