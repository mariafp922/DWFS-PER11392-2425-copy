...
function greyConverter() {
    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            let sumatoria = pixels[i][j][0] + pixels[i][j][1] + pixels[i][j][2];
            let media = Math.round(sumatoria / 3);
            pixels[i][j][0] = media; //R
            pixels[i][j][1] = media; //G
            pixels[i][j][2] = media; //B
        }
    }
...
function dimBrightness(dimFactor) {
    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            pixels[i][j][0] = Math.round(pixels[i][j][0]  / dimFactor); //R
            pixels[i][j][1] = Math.round(pixels[i][j][1]  / dimFactor); //G
            pixels[i][j][2] = Math.round(pixels[i][j][2]  / dimFactor); //B
        }
    }
...
function invertColors() {
    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            pixels[i][j][0] = Math.abs(255 - pixels[i][j][0]); //R
            pixels[i][j][1] = Math.abs(255 - pixels[i][j][1]); //G
            pixels[i][j][2] = Math.abs(255 - pixels[i][j][2]); //B
        }
    }
...
function merge(alphaFirst, alphaSecond) {
    ...
    for (let i = 0; i < catPixels.length; i++) {
        ...
        for (let j = 0; j < catPixels[i].length; j++) {
            let newPixel = [];
            let r = Math.round(catPixels[i][j][0] * alphaFirst + dogPixels[i][j][0] * alphaSecond); //R
            let g = Math.round(catPixels[i][j][1] * alphaFirst + dogPixels[i][j][1] * alphaSecond); //G
            let b = Math.round(catPixels[i][j][2] * alphaFirst + dogPixels[i][j][2] * alphaSecond); //B
            newPixel.push(r);
            newPixel.push(g);
            newPixel.push(b);
