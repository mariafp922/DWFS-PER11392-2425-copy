...
function greyConverter() {
  ...
  for (let i = 0; i < pixels.length; i++) {
    let newRow = [];
    for (let j = 0; j < pixels[i].length; j++) {
      let avgPixel = Math.trunc((pixels[i][j][0] + pixels[i][j][1] + pixels[i][j][2])/3);

      let pixel = [avgPixel, avgPixel, avgPixel];
      newRow.push(pixel);
    }
    newPixels.push(newRow);
  }
...
function blackAndWhiteConverter() {
  ...
  for (let i = 0; i < pixels.length; i++) {
    ...
    for (let j = 0; j < pixels[i].length; j++) {
      let avgPixel = Math.trunc((pixels[i][j][0] + pixels[i][j][1] + pixels[i][j][2])/3);
      let finalPixel = avgPixel < 128 ? 0 : 255;
      let pixel = [ finalPixel, finalPixel, finalPixel];
      newRow.push(pixel);
    }
    newPixels.push(newRow);
  }
...
function dimBrightness(dimFactor) {
  ...
  for (let i = 0; i < pixels.length; i++) {
    let newRow = [];
    for (let j = 0; j < pixels[i].length; j++) {
      let pixel = [(pixels[i][j][0]/2),(pixels[i][j][1]/2),(pixels[i][j][2]/2)];
      newRow.push(pixel);
    }
    newPixels.push(newRow);
  }
...
function merge(alphaFirst, alphaSecond) {
  ...
  for (let i = 0; i < catPixels.length; i++) {
    ...
    for (let j = 0; j < catPixels[i].length; j++) {
      let pixel = [
          (catPixels[i][j][0]*alphaSecond)+(dogPixels[i][j][0]*alphaFirst),
          (catPixels[i][j][1]*alphaSecond)+(dogPixels[i][j][1]*alphaFirst),
          (catPixels[i][j][2]*alphaSecond)+(dogPixels[i][j][2]*alphaFirst)
      ];
      newRow.push(pixel);
    }
    newPixels.push(newRow);
