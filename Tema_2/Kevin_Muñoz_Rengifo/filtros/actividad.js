...
function greyConverter() {
  ...
  pixels.forEach((row) => {
    row.forEach((pixel) => {
      const media = pixel.reduce((previousValue, currentValue) => currentValue += previousValue) / 3
      pixel[0] = media; // R
      pixel[1] = media; // G
      pixel[2] = media; // B
    });
...
function blackAndWhiteConverter() {
  ...
  pixels.forEach((row) => {
    row.forEach((pixel) => {
      const media = pixel.reduce((previousValue, currentValue) => currentValue += previousValue) / 3
      pixel[0] = media < 128 ? 0 : 255; // R
      pixel[1] = media < 128 ? 0 : 255; // G
      pixel[2] = media < 128 ? 0 : 255; // B
    });
  })
...
function scaleDown() {
  ...
  pixels = pixels.filter((_row, index) => index % 2 === 0).map((row) => row.filter((_pixel, index) => index % 2 === 0));
...
function merge(alphaFirst, alphaSecond) {
  ...
  pixels = catPixels.map((row, indexRow) => row.map((pixel, indexPixel) => [
    (pixel[0] * alphaSecond) + (dogPixels[indexRow][indexPixel][0] * alphaFirst), // R
    (pixel[1] * alphaSecond) + (dogPixels[indexRow][indexPixel][1] * alphaFirst), // G
    (pixel[2] * alphaSecond) + (dogPixels[indexRow][indexPixel][2] * alphaFirst) // B
  ]))
