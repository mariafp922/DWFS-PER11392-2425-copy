...
function blackAndWhiteConverter() {
  ...
  for (let i = 0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
      let average = (pixels[i][j][0] + pixels[i][j][1] + pixels[i][j][2]) / 3;
      let colorPixel = 0;
      if (average > 128) colorPixel = 255;
      pixels[i][j] = [colorPixel, colorPixel, colorPixel];
    }
  }
  handler.savePixels(pixels, outputPath);
}
...
function scaleDown() {
  ...
  for (let i = 0; i < pixels.length; i += 2) {
    ...
    for (let j = 0; j < pixels[i].length; j += 2) {
      pixelsImage.push(pixels[i][j]);
    }
    scaledPixels.push(pixelsImage);
  }
...
function merge(alphaFirst, alphaSecond) {
  ...
  for (let i = 0; i < catPixels.length; i++) {
    ...
    for (let j = 0; j < catPixels[i].length; j++) {
      ...
      let r = pixelCat[0] * alphaSecond + pixelDog[0] * alphaFirst;
      let g = pixelCat[1] * alphaSecond + pixelDog[0] * alphaFirst;
      let b = pixelCat[2] * alphaSecond + pixelDog[0] * alphaFirst;
      pixels[i][j] = [r, g, b];
    }
  }
