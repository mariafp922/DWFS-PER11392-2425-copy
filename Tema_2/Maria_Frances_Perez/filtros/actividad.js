...
function greyConverter() {
  ...
  for (let fila = 0; fila < pixels.length; fila++) {
    for (let columna = 0; columna < pixels[fila].length; columna++) {
      let suma = 0;
      //En la primera iteración se calculala suma total
      for (let color = 0; color < 3; color++) {
        suma = suma + pixels[fila][columna][color];
      }
      let media = suma / 3;
      //Asignar media a todos los píxeles
      for (let color = 0; color < 3; color++) {
        pixels[fila][columna][color] = media;
      }
    }
  }
...
function blackAndWhiteConverter() {
  ...
  for (let fila = 0; fila < pixels.length; fila++) {
    for (let columna = 0; columna < pixels[fila].length; columna++) {
      ...
      let valorasignar = 255;
      if (suma / 3 < 128) {
        valorasignar = 0;
      }
      //Asignar valor a todos los píxeles
      for (let color = 0; color < 3; color++) {
        pixels[fila][columna][color] = valorasignar;
      }
    }
  }
  handler.savePixels(pixels, outputPath);
...
function dimBrightness(dimFactor) {
  ...
  for (let fila = 0; fila < pixels.length; fila++) {
    for (let columna = 0; columna < pixels[fila].length; columna++) {
      //Asignar valor dividido por factor a cada píxel
      for (let color = 0; color < 3; color++) {
        pixels[fila][columna][color] = pixels[fila][columna][color] / dimFactor;
      }
    }
  }
  handler.savePixels(pixels, outputPath);
...
function invertColors() {
  ...
  for (let fila = 0; fila < pixels.length; fila++) {
    for (let columna = 0; columna < pixels[fila].length; columna++) {
      //Asignar valor dividido por factor a cada píxel
      for (let color = 0; color < 3; color++) {
        pixels[fila][columna][color] = 255 - pixels[fila][columna][color];
      }
    }
  }
  handler.savePixels(pixels, outputPath);
...
function merge(alphaFirst, alphaSecond) {
  ...
  //Aqui tu codigo
  //Como so iguales se va utilizar uno de ellos para recorerlos
  for (let fila = 0; fila < catPixels.length; fila++) {
    let filaAux = [];
    for (let columna = 0; columna < catPixels[fila].length; columna++) {
      let valoresAux = [];
      valoresAux = [
        catPixels[fila][columna][0] * alphaSecond +
          dogPixels[fila][columna][0] * alphaFirst,
        catPixels[fila][columna][1] * alphaSecond +
          dogPixels[fila][columna][1] * alphaFirst,
        catPixels[fila][columna][2] * alphaSecond +
          dogPixels[fila][columna][2] * alphaFirst,
      ]; //Asignar valor dividido por factor a cada píxel
      filaAux.push(valoresAux);
    }
    pixels.push(filaAux);
  }
