...
function suggest(numSeats) {
  ...
  if (numSeats > numCols) return suggestSeats;
...
const buttonJoke = document.querySelector("#btn-confirm");
buttonJoke.addEventListener("click", (event) => {
  event.preventDefault();
  let numSeats = document.querySelector("#input-quantity")?.value ?? 1;

  // Sugerir x asientos
  const suggestSeats = suggest(Number(numSeats)).join(",");
  document.querySelector("#seats").innerHTML = suggestSeats;
});
